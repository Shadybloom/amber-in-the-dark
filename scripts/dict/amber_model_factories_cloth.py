#----
# Заметки:
    # Ткань (шёлковая) -- 144 грамм/м² 45 нормо-часов/кв.метр (считая сбор листьев шелковицы)
    # Ткань (шерстяная) -- 1 кг/день, 5 кв.метров/день (2 нормо-часа/кв.метр)
    # Ткань (шерстяная тяжёлая) -- 4 кг/день, 4.5 кв.метров/день (2.2 нормо-часа/кв.метр)
    # Ткань (шерстяная крестьянская) -- <0.17 кг/день, 0.83 кв.метров/день (6.7 нормо-часа/кв.метр)
...
# Производства (одежда)
    ...
    # Производство одежды:
        # Брюки (фабрика) -- 2.5 нормо-часов
        # Жилеты (фабрика) -- 0.37 нормо-часа
        # Жакеты (фабрика) -- 0.92 нормо-часа
        # Костюмы (фабрика) -- 8 нормо-часов
        # Пальто (фабрика) -- 8 нормо-часов
        # Рубаха (ручной труд) -- 4 нормо-часов
        # https://istmat.org/node/27433
    # Производство одежды (18 век):
        # Shirt (2.7 кв.метра) -- 3 нормо-часов
        # Vest (1.7 кв.метра) -- 2 нормо-часа
        # Breeches (2.1 кв.метра) -- 5 нормо-часов
        # Overcoat (5.9 кв.метра) -- 8 нормо-часов
        # Sweater (5 кв.метров) -- 8 нормо-часов
        # Шуба (55-60 кроличьих шкурок) -- 16 нормо-часов
        # "Grain into Gold" стр 22, стр 28
    # Износ:
...
metadict_detail['_Производство бязи (квадратный метр)'] = {
        # Бязь -- 120 грамм/кв.метр
        # В мастерской 100 рабочих (две смены), 100 станков, 160k нормо-часов/год
            # Единороги в наушниках. Галерея над станками, чтобы пылью не дышать.
            # Механический станок = ×2.5 ручного, ткач на 4-8 станков.
...
metadict_detail['_Производство бархата (квадратный метр)'] = {
        ...
        # Плюш, вельвет (исторически делали из шёлка)
        # бархат -- 300 грамм/кв.метр
...
metadict_detail['_Производство парусины (квадратный метр)'] = {
        # - Фабричная (механический станок) -- 0.7-1.4 нормо-часа/кв.метр
...
metadict_detail['_Производство соломенных ковров (квадратный метр)'] = {
        ...
        # Ткётся двумя рабочими по 80-130 аршин/день, (57-92 метра, 50-82 кв.метра)
            # Производительность труда -- 0.25-0.4 нормо-часа/кв.метр
...
# Производства (пряжа)
    # Производительность:
        ...
        # Трудоэффективность (Китай, 1913) -- 1 рабочий на 170 веретен
        # Трудоэффективность (США, 1913) -- 1 рабочий на 900 веретен
...
metadict_detail['_Производство хлопчатой пряжи (килограмм)'] = {
        # Техпроцессы:
            ...
            # 6) предпрядение ровницы на толстопрядильной машине (40-100 веретен)
                # Размер катушек ровницы (1890 год) -- 1311 см³
            # 7) прядение на веретенах
                # Размер катушек пряжи (1890 год) -- 230 см³
            ...
            # https://dic.academic.ru/dic.nsf/bse/124740/Прядильное
        # Кольцевая прядильная машина, работы:
            # 1. Присучивание. Соединение концов оборванной нити.
            # 2. Заправка катушек в шпулярник. Замена катушек.
            # 3. Очистка. Хлопковых волокон с прядильной рамы.
            # 4. Съемка. Удаление наработанных катушек с пряжей.
                # Бригадой съемщиков, каждые 3 часа, 3.3 секунды/катушка.
                # Расходуется 2-4/200 минут, одним рабочим 20/200 минут.
            # 5. Обход. Наблюдение за машинами для операций 1–3.
...
metadict_detail['_Производство хлопка-сырца (килограмм)'] = {
        # Коттон-джин, лучший (1000 рублей, 70 пил, 30 кВт) -- 2500 кг сырца в сутки (600 кг/час сырья)
            ...
            # Обработка хлопка-сырца коттон-джином -- 0.44 нормо-часа/кг (50 фунтов/день)
            # Обработка хлопка-сырца вручную -- 22 нормо-часа/кг (1 lb/день)
            # "Grain into Gold" стр 27
